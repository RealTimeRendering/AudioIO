function Knob( element, options ) {
    ...
    this._makeElements( options );

    // var size = this.element.className.match( /(large|medium|small|tiny)/g );
    this.elementSize = options.size ? options.size : 'large';
    ...
    this._setAttribute( 'step', this.attributes.exponent === 1 ? this.deltaMultiplier : 1 );
    ...
    this.steppedAngle = this._roundToMultiple( this.angle, this.angleStep );
...
Knob.prototype._makeElements = function( options ) {
    ...
    knob.className = 'knob ' + options.type + ' ' + options.size;
...
Knob.prototype._setAttribute = function( attrName, defaultValue ) {
    ...
    if ( typeof attrValue !== 'string' ) {
        attrValue = typeof defaultValue === 'function' ? defaultValue.call( this ) : defaultValue;
    }
    else {
...
Knob.prototype._setValue = function() {
    ...
    this.value = this._clamp( this.value, attrs.min, attrs.max );
    this.steppedValue = this._roundToMultiple( this.value, attrs.step );
    this.steppedValue = this._clamp( this.steppedValue, attrs.min, attrs.max );
};
